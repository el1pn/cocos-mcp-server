...
export class PreferencesTools implements ToolExecutor {
    ...
    private async openPreferencesSettings(tab?: string, args?: any[]): Promise<ToolResponse> {
        ...
        return toolCall(
            () => editorRequest('preferences', 'open-settings', ...requestArgs),
    ...
    private async queryPreferencesConfig(name: string, path?: string, type: string = 'global'): Promise<ToolResponse> {
        ...
        return toolCall(
            () => editorRequest<any>('preferences', 'query-config', ...requestArgs),
            (config) => ({
                data: {
                    name: name,
                    path: path,
    ...
    private async setPreferencesConfig(name: string, path: string, value: any, type: string = 'global'): Promise<ToolResponse> {
        return toolCall(
            () => editorRequest<boolean>('preferences', 'set-config', name, path, value, type),
            (success) => {
                if (success) {
        ...
        return toolCall(
            async () => {
                ...
                const queryPromises = categories.map(category => {
                    return editorRequest('preferences', 'query-config', category, undefined, 'global')
                        .then((config: any) => {
    ...
    private async resetPreferences(name?: string, type: string = 'global'): Promise<ToolResponse> {
        ...
        return toolCall(
            async () => {
                const defaultConfig = await editorRequest('preferences', 'query-config', name, undefined, 'default');
                return editorRequest<boolean>('preferences', 'set-config', name, '', defaultConfig, type);
            },
            (success) => {
                if (success) {
